...
// ---------- Mostrar productos en versión mobile-tablet
function versionMobile(mobile) {
    let tresProductos = productos.slice(9, 11);
    let dosProductos = productos.slice(9, 12);

    if (mobile.matches) {
        mostrarProductos(tresProductos);
    } else {
        mostrarProductos(dosProductos);
    }
}

const mobile = window.matchMedia("(max-width: 768px)");
versionMobile(mobile);
mobile.addListener(versionMobile)
